...
4. When prompted, pick your Datadog domain from the **Datadog Domain** dropdown (e.g. `mcp.datadoghq.com` for US1, `mcp.datadoghq.eu` for EU). The plugin uses this to connect to the right Datadog site. If you skip this step, the plugin defaults to US1 (`mcp.datadoghq.com`).
...
- `DD_MCP_DOMAIN` — the Datadog MCP domain. This is the same value you pick from the **Datadog Domain** dropdown when installing the plugin; Cursor sets it for you. If Cursor does not inject the plugin variable (for example in web Automations), the plugin falls back to `mcp.datadoghq.com` so OAuth can still start. You can also set it manually in your shell for non-standard environments or key authentication.
